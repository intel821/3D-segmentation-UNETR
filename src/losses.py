...
class Dice_and_BCE_Loss(nn.Module):
    def __init__(self,gamma=2):
        super(Dice_and_BCE_Loss,self).__init__()
        self.dice_loss = DiceLoss()
        #self.focal_loss = FocalLoss(gamma)

    def forward(self,input, target):
        loss = self.dice_loss(input, target)  + nn.functional.binary_cross_entropy(input,target)
        return loss
...
class BCE_Loss(nn.Module):
    ...
    def forward(self,input,target):
        loss = nn.functional.binary_cross_entropy(input,target)
        return loss